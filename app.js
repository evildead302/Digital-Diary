...
function renderTemp() {
    ...
    tempEntries.forEach((e, i) => {
        const li = document.createElement("li");
        li.className = e.amount < 0 ? "expense" : "income";
        li.innerHTML = `
            <div class="temp-entry">
                <span class="date">${e.date}</span>
                <span class="desc">${e.desc}</span>
                <span class="category">${e.main} → ${e.sub}</span>
                <span class="amount ${e.amount < 0 ? 'negative' : 'positive'}">
                    ${e.amount < 0 ? '-' : '+'}₹${Math.abs(e.amount).toFixed(2)}
                </span>
                <div class="actions">
                    <button onclick="editTemp(${i})" class="small-btn">✏️ Edit</button>
...
function editTemp(i) {
    ...
    if (entryMain) {
        ...
        // Need to wait for dropdown to update
        setTimeout(() => {
        ...
        recentEntries.forEach(e => {
            ...
            li.innerHTML = `
                <div class="saved-entry">
                    ...
                    <div class="actions">
                        <button onclick="editSaved('${e.id}')" class="small-btn">✏️ Edit</button>
...
function calcTotal() {
    getAllEntries(d => {
        ...
        if (totalBalance) totalBalance.innerText = `₹${total.toFixed(2)}`;
        if (totalIncome) totalIncome.innerText = `₹${income.toFixed(2)}`;
        if (totalExpense) totalExpense.innerText = `₹${expense.toFixed(2)}`;
        if (netBalance) netBalance.innerText = `₹${total.toFixed(2)}`;
        ...
        // Load recent entries on home page
        loadRecentEntries(d);
    });
}
...
function loadRecentEntries(entries) {
    ...
    recent.forEach(e => {
        ...
        li.innerHTML = `
            ...
            <span class="recent-desc">${e.desc}</span>
            <span class="recent-amount ${e.amount < 0 ? 'negative' : 'positive'}">
                ${e.amount < 0 ? '-' : '+'}₹${Math.abs(e.amount).toFixed(2)}
            </span>
        `;
    ...
    getAllEntries(d => {
        ...
        if (statsElement) {
            statsElement.innerHTML = `
                ...
                Total: <span style="color:${total >= 0 ? '#2e7d32' : '#c62828'}">₹${total.toFixed(2)}</span> |
                Income: <span style="color:#2e7d32">₹${incomeTotal.toFixed(2)}</span> |
                Expense: <span style="color:#c62828">₹${expenseTotal.toFixed(2)}</span>
            `;
        }
        ...
        filtered.forEach(e => {
            ...
            row.innerHTML = `
                <td>${e.date}</td>
                <td>${e.desc}</td>
                <td><span class="category-badge">${e.main}</span></td>
                <td><span class="subcategory-badge">${e.sub}</span></td>
...
function displaySearchResults(results, searchTerm = "") {
    ...
    results.forEach(entry => {
        const li = document.createElement("li");
        li.className = `transaction-item ${entry.amount >= 0 ? "income" : "expense"}`;
        ...
        li.innerHTML = `
            ...
            <div class="transaction-category">
                <span class="main-cat">${entry.main}</span>
                <span class="sub-cat">→ ${entry.sub}</span>
            </div>
            <div class="transaction-actions">
                <button onclick="editSaved('${entry.id}')" class="small-btn">✏️ Edit</button>
...
function clearSearch() {
    ...
    document.getElementById("searchTotal").textContent = "💰 Total: ₹0.00";
}
...
async function testNeonConnection() {
    ...
    if (testResult) {
        testResult.innerHTML = '<span style="color: #2196f3;">🔍 Testing connection...</span>';
    }
    
    try {
        // Simulate connection test (in real app, you'd make actual API call)
        await new Promise(resolve => setTimeout(resolve, 1000));
        
        if (testResult) {
            testResult.innerHTML = '<span style="color: #4caf50;">✅ Connection successful!</span>';
        }
        
        // Update UI
        updateGlobalNeonStatus();
        
        alert("✅ Neon connection test successful!");
        
    } catch (error) {
        if (testResult) {
            testResult.innerHTML = '<span style="color: #f44336;">❌ Connection failed</span>';
        }
...
async function uploadToNeon() {
    ...
    try {
        getAllEntries(async entries => {
            ...
            console.log("Uploading to Neon:", unsyncedEntries);
            ...
            setTimeout(() => {
                ...
                updateGlobalNeonStatus();
                alert(`✅ Successfully synced ${unsyncedEntries.length} entries to Neon!`);
                loadSaved();
            }, 1500);
...
async function restoreFromNeon() {
    ...
    try {
        ...
        calcTotal();
        loadSaved();
        alert(`✅ Restored ${mockRemoteData.length} sample entries from Neon!`);
...
function updateBackupStatus() {
    getAllEntries(entries => {
        ...
        if (dbInfo) {
            dbInfo.textContent = `Total Entries: ${entries.length} | Last Backup: ${neonConfig ? "Configured" : "Not configured"}`;
        }
        updateGlobalNeonStatus();
    });
}
...
function updateOnlineStatus() {
    ...
    if (statusElement) {
        if (navigator.onLine) {
            console.log('✅ Online');
            // Try to sync if there are pending entries
            if (neonConfig) {
                getAllEntries(entries => {
                    const pending = entries.filter(e => !e.synced).length;
                    if (pending > 0) {
                        console.log(`🔄 ${pending} pending entries to sync`);
                        // Auto-sync when coming online
                        if (document.getElementById("autoSync")?.checked) {
                            setTimeout(uploadToNeon, 3000);
                        }
                    }
                });
            }
            ...
            if (statusElement) {
                ...
                // Restore original status when back online
...
window.addEventListener('beforeinstallprompt', (e) => {
    ...
    installButton.className = 'primary-btn';
    ...
    if (homePage) {
        homePage.appendChild(installButton);
    }
});
...
if (isRunningStandalone()) {
    ...
    // You can adjust UI for standalone mode if needed
}
...
function exportAllData() {
    ...
    const exportFileDefaultName = `accounts_diary_backup_${new Date().toISOString().split('T')[0]}.json`;
