...
// ====================
// TEMPORARY ENTRIES - FIXED TABLE LAYOUT
// ====================
...
function renderTemp() {
    ...
    tempEntries.forEach((e, i) => {
        ...
        row.className = `${i % 2 === 0 ? 'even' : 'odd'} ${e.amount < 0 ? "expense-row" : "income-row"}`;
...
// ====================
// SAVED ENTRIES MANAGEMENT - FIXED TABLE LAYOUT
// ====================
...
function displaySearchResults(results, searchTerm = "") {
    ...
    results.forEach((entry, i) => {
        const row = document.createElement("tr");
        row.className = `${i % 2 === 0 ? 'even' : 'odd'} ${entry.amount >= 0 ? "income-row" : "expense-row"}`;
...
// ====================
// NEON FUNCTIONS (Fixed to avoid recursion)
// ====================
...
async function testNeonConnection() {
    ...
    try {
        // Call the Neon function from neon.js
        // First, ensure neonSync is available
...
async function uploadToNeon() {
    ...
    try {
        // First check table exists - call neonSync.testConnection directly
        if (typeof neonSync === 'undefined' || !neonSync.testConnection) {
            throw new Error("Neon module not loaded properly");
        }
        
        const tableResult = await neonSync.testConnection();
            ...
            for (const entry of entriesToSync) {
                try {
                    if (entry.syncRemarks === "deleted") {
                        ...
                        const uploadResult = await neonSync.uploadEntries([entry]);
...
async function restoreFromNeon() {
    ...
    try {
        // Check table exists - call neonSync.testConnection directly
        if (typeof neonSync === 'undefined' || !neonSync.testConnection) {
            throw new Error("Neon module not loaded properly");
        }
