...
function renderTemp() {
    ...
    tempEntries.forEach((e, i) => {
        ...
        row.className = `${i % 2 === 0 ? 'even' : 'odd'} ${e.amount < 0 ? "expense" : "income"}`;
...
function loadSaved() {
    getAllEntries(d => {
        ...
        table.appendChild(tbody);
        savedEntryList.appendChild(table);
    });
}
...
function loadRecentEntries(entries) {
    ...
    table.appendChild(tbody);
    recentEntries.appendChild(table);
}
...
function displaySearchResults(results, searchTerm = "") {
    ...
    results.forEach((entry, i) => {
        const row = document.createElement("tr");
        row.className = `${i % 2 === 0 ? 'even' : 'odd'} ${entry.amount >= 0 ? "income" : "expense"}`;
...
async function uploadToNeon() {
    ...
    try {
        ...
        const tableResult = await testNeonConnection();
